...
export const tabsItem = (tabInfo) => {
    ...
    li.elButton = doElement(`<button id="${tabId}" class="dds__tabs__tab" role="tab" aria-controls="${tabId}-pane" aria-selected="${tabSelected}" tabindex="0">`);
    li.elButtonSpan = doElement(`<span class="dds__tabs__tab__label" title="${tabLabel}">${tabLabel}</span>`);
